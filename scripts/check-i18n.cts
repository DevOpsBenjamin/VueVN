...
async function main() {
  ...
  console.log(`🔍 Checking i18n completeness for project: ${projectInfo.projectName}`);
  ...
  if (fs.existsSync(locationsPath)) {
    ...
    for (const location of locations) {
      ...
      if (fs.existsSync(locationTextsPath)) {
        await checkTextsDirectory(locationTextsPath, `locations.${location}`, missingTranslations);
      }
    }
  }
  ...
  if (missingTranslations.length === 0) {
    ...
    for (const missing of missingTranslations) {
      for (const lang of missing.missingLanguages) {
        languageMissingCount.set(lang, (languageMissingCount.get(lang) || 0) + 1);
      }
    }
...
async function checkTextsDirectory(dirPath: string, basePath: string, missingTranslations: MissingTranslation[]): Promise<void> {
  ...
  for (const dir of directories) {
    ...
    if (languageFiles.length > 0) {
      ...
      await checkLanguageFiles(subDirPath, currentPath, languageFiles, missingTranslations);
    }
...
async function checkLanguageFiles(dirPath: string, textPath: string, languageFiles: string[], missingTranslations: MissingTranslation[]): Promise<void> {
