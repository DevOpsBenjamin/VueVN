...
import { generateActionsFile, generateEventsFile, generateLocationIndex, generateGlobalIndex } from './lib/location-generator';
...
const globalPath = path.join(projectPath, 'global');
...
try {
  ...
  if (fs.existsSync(locationsPath)) {
    ...
    for (const locationName of locationFolders) {
      ...
      await generateActionsFile(projectName!, projectPath, locationName, locationGeneratePath, false);

      // Generate events.ts for the location
      await generateEventsFile(projectName!, projectPath, locationName, locationGeneratePath, false);

      // Generate index.ts for the location (after actions/events are created)
      await generateLocationIndex(projectName!, projectPath, locationName, locationGeneratePath);
    }
  } else {
  ...
  await generateActionsFile(projectName!, projectPath, 'global', generateGlobalPath, true);

  // Generate events.ts for the global  
  await generateEventsFile(projectName!, projectPath, 'global', generateGlobalPath, true);
