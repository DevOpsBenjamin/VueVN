...
function getAllFiles(dir: string, fileExtension: string, basePath: string = ''): string[] {
...
export async function generateDifferentialExports(config: GeneratorConfig): Promise<void> {
  ...
  const engineResourcePath = path.join(process.cwd(), 'engine_src', config.resourceType);
  const projectResourcePath = path.join(projectPath, 'plugins', config.resourceType);
  const generatePath = path.join(process.cwd(), 'generate');

  console.log(`🏗️  Generating ${config.resourceType} for project: ${projectName}`);
  ...
  try {
    ...
    const engineResources = getAllFiles(engineResourcePath, config.fileExtension);
    console.log(`🔧 Found ${engineResources.length} engine ${config.resourceType}: ${engineResources.join(', ')}`);

    // Get all project resources
    const projectResources = getAllFiles(projectResourcePath, config.fileExtension);
    if (projectResources.length > 0) {
      console.log(`🎯 Found ${projectResources.length} project ${config.resourceType}: ${projectResources.join(', ')}`);
    } else {
      console.log(`📝 No project ${config.resourceType} found - using engine ${config.resourceType} only`);
    }
    ...
    for (const resource of projectResources) {
      if (resourceSourceMap.has(resource)) {
        console.log(`🔄 Project overrides engine ${config.resourceType.slice(0, -1)}: ${resource}`);
      } else {
        console.log(`➕ Project adds new ${config.resourceType.slice(0, -1)}: ${resource}`);
      }
    ...
    if (config.exportType === 'type') {
      ...
      fileContent = `// Generated ${config.resourceType} index - differential ${config.resourceType.slice(0, -1)} system
...
`;
    ...
    console.log(`✅ ${config.resourceType.charAt(0).toUpperCase() + config.resourceType.slice(1)} generation complete`);

  } catch (error: any) {
    console.error(`❌ ${config.resourceType.charAt(0).toUpperCase() + config.resourceType.slice(1)} generation failed:`, error.message);
    process.exit(1);
  }
}