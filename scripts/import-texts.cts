...
async function validateImport(tempDir: string, projectPath: string): Promise<void> {
  ...
  if (fs.existsSync(locationTexts) && fs.existsSync(originalLocationTexts)) {
    const locations = fs.readdirSync(locationTexts)
      .filter(item => fs.statSync(path.join(locationTexts, item)).isDirectory());
    
    for (const location of locations) {
      const importedLocation = path.join(locationTexts, location, 'texts');
      const originalLocation = path.join(originalLocationTexts, location, 'texts');
...
async function validateDirectoryStructure(importDir: string, originalDir: string): Promise<void> {
...
async function importTexts(tempDir: string, projectPath: string): Promise<void> {
  ...
  if (fs.existsSync(locationSource)) {
    const locations = fs.readdirSync(locationSource)
      .filter(item => fs.statSync(path.join(locationSource, item)).isDirectory());
...
async function copyTextFiles(sourceDir: string, targetDir: string): Promise<void> {
...
async function restoreBackup(backupPath: string, projectPath: string): Promise<void> {
